...
Assignment: Lab9 C. Velocity and moving objects

Let’s consider an object moving in 3D space. We already know how to
describe its position. (We will be assuming metric system, thus distances
will be implicitly measured in meters and time in seconds.)
...
When moving at constant velocity v, the object’s new position after the
elapsed time dt can be computed as
...
Because we pass the coordinates Coord3D * ppos as a pointer, all changes
to the fields of the class pointed by ppos, will affect the original object
you pass into the function, not its local copy. Example:
...
Notice that we are not passing anything by reference: We pass the address,
&pos, and the function manipulates the original object pos, because it
knows its address in the memory.

On dynamic memory allocation
Normally, any variable “lives” only within the block where it is declared,
and disappears once the program execution leaves this scope. We know that
already, right?

This memory management is called automatic, the program allocates memory
for each variable when it enters the scope of the variable, and deletes
that memory when leaving that scope.
...
and freed by the program. Even though we returned the address where the
poem was, after the function exits, that address may be taken and used
by some other part of your program, the poem may be easily overwritten
by some other value.
...
released to the system, otherwise if we only keep allocating memory and
never giving it back, we may run out of memory eventually.
